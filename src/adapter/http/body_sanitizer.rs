...
impl HttpBodySanitizer {
    ...
    /// URL-encoded forms and text bodies render the available prefix with a
    ...
    fn sanitize_body_inner(
        ...
        if content_type.is_some_and(content_type::is_multipart) {
            ...
            if let Some(text) = multipart::sanitize_multipart(self, content_type, bytes) {
                return text;
            }
    ...
    pub(super) fn sanitize_json(&self, bytes: &[u8]) -> Option<String> {
    ...
    pub(super) fn sanitize_ndjson(&self, bytes: &[u8]) -> Option<String> {
    ...
    pub(super) fn sanitize_form(&self, bytes: &[u8]) -> String {
