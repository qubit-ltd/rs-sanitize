...
HTTP header、argv 向量和环境变量。adapter 只解析自己明确建模的格式；shell
...
- 提供 URL、URL-encoded form、HTTP header、argv 向量和环境变量 adapter
...
`HttpHeaderSanitizer` 处理 `http::HeaderMap` 和 `http::HeaderValue`，但 body
preview、content type、JSON 和 multipart
